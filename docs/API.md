...
| GET | `/payroll/runs?status` | MANAGE_PAYROLL | Every payroll run for this salon, newest first. |
| GET | `/payroll/runs/:id` | MANAGE_PAYROLL | One run, with its frozen per-staff `lines`. |
| POST | `/payroll/runs` | MANAGE_PAYROLL | `{ periodStart, periodEnd }` — submits a run covering every staff member with an employment profile. Idempotent on the money (same shape as `POST /incentive-payouts`, DECISIONS.md §66): an unchanged total returns the existing run; a moved total voids the old run and submits a fresh one. `409 NO_ELIGIBLE_STAFF` if nobody has an employment profile yet; `409 PAYROLL_RUN_ALREADY_PAID` refuses to supersede a run that's already been paid. A statutory line only appears per staff member when the tenant's statutory calculations are enabled, a rule set is published, and the period is exactly one calendar month — otherwise every line's `statutory` is `null`. |
