...
describe("resolveModules", () => {
  it("PRO defaults every module on", () => {
    ...
    expect(modules).toEqual({ attendance: true, incentives: true, reports: true, auditLog: true, invoices: true });
  });

  it("LITE defaults every module off", () => {
    const modules = resolveModules(entitlements({ tier: "LITE" }));
    expect(modules).toEqual({ attendance: false, incentives: false, reports: false, auditLog: false, invoices: false });
  });
