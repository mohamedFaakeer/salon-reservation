...
4. **Holds:** 10-minute temporary slot hold. Successful payment converts hold → confirmed appointment in the **same transaction**. Expired holds are released lazily — the next conflicting write for that staff member (tenant-scoped) marks a stale `HELD` row `EXPIRED` before re-checking the exclusion constraint (`booking.service.ts`); there is no separate scheduled sweeper job (SECURITY_AUDIT_REPORT.md F-04).
...
9. **Pushing new migrations is not the same as running them.** Before/when pushing any change that adds or edits a file under `apps/api/src/infrastructure/database/migrations/`, explicitly check whether production (Neon, via Render) is behind: compare the migration files in the push against what's actually been applied to prod. A migration sitting only in git is a live outage waiting to happen the moment the matching code deploys (a route/service that queries a table that doesn't exist yet in prod) — this already happened three times now (customer-auth signup; separately a full notifications-engine migration set; and a session-timeout policy's `refresh_session` migration, all shipped to prod code without their migrations ever being run). Flag it to the user and get their go-ahead to run `npm run db:migrate:prod` (or walk them through doing it) as part of that same push — don't treat "pushed to main" as "done" when a migration is part of the change.
   **How to actually run it, every time:** `npm run db:migrate:prod`'s migration runner reads migration files from whatever is on disk in the *current local checkout* — it has no idea what's deployed. Running it from a branch that doesn't contain the new migration file silently reports "no migrations are pending" and does nothing, while production stays broken (this is exactly how the third incident above happened). Before running it: `git checkout main && git pull origin main` first — checkout alone isn't enough, local `main` can also be stale relative to `origin/main` — then run the command, and glance at the `DATABASE_URL` being used to confirm it's actually production and not a local/dev value.
10. **Confirm the branch before pushing.** Before running `git push`, state the current local branch and what's about to be pushed, and get explicit confirmation first — don't push immediately just because a push was requested, even when the target branch was named explicitly, since the local checkout doing the pushing could still be on something else.
