  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {wizard.loadingSlots ? (
        ...
        <EmptyState title={wizard.slotsError} action={{ label: "Retry", onClick: () => wizard.setSelectedDate(wizard.selectedDate) }} />
      ) : wizard.slots.length === 0 ? (
        <EmptyState title={`No open slots on ${wizard.selectedDate} — try another date or staff.`} />
      ) : (
          ...
          {wizard.slots.map((slot, i) => (
            <li key={`${slot.staffId}-${slot.start}`}>
              ...
              >
                ...
                {i === 0 ? <span className="mt-1 text-xs font-medium text-teal-700">Fastest available</span> : null}
              </button>
            </li>
          ))}
        </ul>
