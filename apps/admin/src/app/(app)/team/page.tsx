...
/** Shown once, right after creating a login — the server keeps only a hash. */
interface NewLogin {
  name: string;
  email: string;
  password: string;
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        <p className="rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700">
          Only the salon owner can create or change logins.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {newLogin ? (
        ...
        >
          <p className="text-sm font-semibold text-teal-900">{newLogin.name} can now sign in.</p>
      ...
      {creating ? (
        <TeamDrawer
          ...
          onCreated={(member, password) => {
            setCreating(false);
            setNewLogin({ name: member.name, email: member.email, password });
