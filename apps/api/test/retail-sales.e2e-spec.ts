...
describe("Retail sales (e2e) — cross-tenant isolation", () => {
  ...
  async function provisionTenant(namePrefix: string): Promise<string> {
    ...
    const ownerEmail = `owner-${unique}@retail-sales-e2e.test`;
    await request(server())
      .post("/api/v1/super-admin/tenants")
      .set("Authorization", `Bearer ${superToken}`)
      .send({
        salonName: `${namePrefix} Salon`,
        slug: `retail-sales-e2e-${unique}`,
  ...
  it("cross-tenant: a sale rung up under one tenant 404s for another tenant's owner", async () => {
    const tenantAOwner = await login("owner@demo.salon", "demo1234");
    // A custom line needs no Product/ProductVariant/stock setup — the same
    // off-catalog "sell something not yet in the catalog" path Quick Sale uses.
    const created = await request(server())
      .post("/api/v1/retail-sales/checkout")
      .set("Authorization", `Bearer ${tenantAOwner}`)
      .set("Idempotency-Key", crypto.randomUUID())
      .send({
        lines: [{ custom: { name: "Cross Tenant Item", unitPriceCents: 500 }, quantity: 1 }],
        paymentMethod: "CASH",
      })
      .expect(201);

    const tenantBOwner = await provisionTenant("Retail Sales Cross Tenant B");
    await request(server())
      .get(`/api/v1/retail-sales/${created.body.id}`)
