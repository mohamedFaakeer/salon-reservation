...
describe("Service packages (e2e) — cross-tenant isolation", () => {
  ...
  async function provisionTenant(namePrefix: string): Promise<string> {
    ...
    const ownerEmail = `owner-${unique}@service-packages-e2e.test`;
    await request(server())
      .post("/api/v1/super-admin/tenants")
      .set("Authorization", `Bearer ${superToken}`)
      .send({
        salonName: `${namePrefix} Salon`,
        slug: `service-packages-e2e-${unique}`,
  ...
  it("cross-tenant: a service package sold under one tenant 404s for another tenant's owner", async () => {
    ...
    const serviceRes = await request(server())
      ...
      .send({ name: `Cross Tenant Package Service ${Date.now()}`, durationMin: 30, priceCents: 500000 })
      .expect(201);
    ...
    const created = await request(server())
      ...
      .send({
        ...
        customer: { firstName: "Cross", lastName: "Tenant", phone: uniquePhone() },
        paymentMethod: "CASH",
      })
      .expect(201);

    const tenantBOwner = await provisionTenant("Service Packages Cross Tenant B");
    await request(server())
      .get(`/api/v1/service-packages/${created.body.id}`)
